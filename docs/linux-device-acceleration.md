...
- CPU、显式 WebGPU FP32 与 D112 Auto 使用不可变的 FP32 原模型；显式 WebGPU FP16 使用转换版本、源/输出哈希均锁定的 native-FP16-I/O 派生模型。
- FP32/FP16 allow 都必须验证实际 WebGPU placement，并把 CPU 算子限制为 `Concat/Gather/Slice`；strict 验证稳定 fail-closed，而不再要求当前图不可能达到的全 WebGPU recognition。Node Auto 和直接 C++ Auto 单独要求实际选择 WebGPU，不能把 CPU fallback 当作 PG 成功。
...
- 默认一键套件覆盖锁定 14-fixture corpus、CPU FP32、WebGPU FP32、WebGPU FP16 allow、strict fail-closed、Auto 与 ORT node placement；每个正常 case 固定 3 次独立 engine cold start、每次 2 次 warmup + 10 次测量（合计 30 次），另做 20 次 FP16 engine lifecycle，并检查质量容差、CPU 算子白名单、冷启动、RSS、256 MiB 解包 native payload 与性能门槛；报告带 sidecar SHA-256，报告内记录 profile hashes，原始 cases、profiles 和日志一并保留。
...
| 拒绝本轮 WebGPU | contract/质量/安全/供应链任一否决项失败；CPU partition 超出锁定的三类算子；strict 未能稳定 fail-closed；包体/driver 维护超出预注册边界；或 FP16 候选仍无目标 workload 通过性能 Gate | 不加入公共 API 或默认包，CPU 与其他厂商路径继续；记录失败层、证据和可测量的重启条件 |
...
- 已完成 C++/Node plugin registration、D112 Auto、FP32/FP16 模型路由、strict fail-closed、descriptor、npm self-contained payload、license/SBOM 与 qualification-only release gate。
- 已完成 FP16 可复现派生与确定性运行验证、Linux/Windows hardware-independent CI 和完整真机 runner；FP16 placement 与质量/收益结论等待真机 profile。
...
- 审查 FP32/FP16 placement、strict rejection、完整质量、性能、内存、生命周期和离线 package Gate。
- 如果 FP16 收益仍不足，先判断 copy/dispatch/shape cache 是否是主因，再决定是否进入 bucket 或 I/O Binding；不为追数字跳过质量 Gate。
...
1. 用户的 Linux 与 Windows GPU/driver 是否都通过 FP32/FP16 placement、FP16 质量、strict rejection、冷启动、性能、RSS 与 lifecycle Gate？
2. 含锁定三类 CPU graph partition 的 FP16 Preview 是否可接受，最低 placement coverage 如何写入兼容范围？
