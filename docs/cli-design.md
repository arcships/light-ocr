...
| 能力 | 现状 | 来源 |
| --- | --- | --- |
| `createEngine` / `OcrEngine.recognize(RawImage)` / `recognizeEncoded(Uint8Array)` / `info` / `close` | 已发布 | [bindings/node/js/index.d.ts](../bindings/node/js/index.d.ts) |
...
monorepo 迁移按 [roadmap §3.4](roadmap.md) 规划在 N2 启动时执行，N1 仍运行在当前单包结构下。CLI 放在 `bindings/node/bin/light-ocr.cjs`，`package.json.bin` 指向它，装 `@arcships/light-ocr` 即得 `light-ocr` 命令——符合 [roadmap §3.1](roadmap.md)「`@arcships/light-ocr` 唯一拥有 `light-ocr` bin、开箱即用」。实现用 Node 内置模块（`fs`/`path`/`process`），零运行时依赖，符合禁网安装与 `--ignore-scripts` 要求；argv 解析手写最小 parser（D-N1-2）。
...
以下在进入实现前需维护者确认，确认后提炼为 [decisions.md](decisions.md) 的 D106。已确认项不再列出（CLI 分发形态 = `bindings/node/bin/` + facade bin；`--region` = 整数像素；`--no-color`/`--quiet` 默认行为 = 非 TTY 或 `NO_COLOR` 自动关闭彩色）。
