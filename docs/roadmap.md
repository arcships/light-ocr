...
| Perf-3 自包含加速分发 | PG 通过后 | 每个 provider 3–5 周 | 通过统一 facade 自动取得的 platform/provider payload 与显式选择 API | 单入口安装、干净 VM 禁网、session fallback/CPU partition、质量和性能 release gates 全绿 |
...
可以利用宿主机器的 GPU、Apple Neural Engine 或 NPU，不必长期只靠 CPU。现有推理层使用 ONNX Runtime；跨平台的低成本候选是在 session 创建时注册 Execution Provider（EP），而不是重写 OCR pipeline 或为每种硬件维护一套算法。Apple 路径已经观察到动态 ORT CoreML graph placement 与静态 Direct CoreML 的明显差异，因此正式 backend 仍需按同条件 Gate 比较后决定。
...
加速线新增一条不可退让的产品约束：用户继续只安装 OS 无关入口 `@arcships/light-ocr`。facade 通过现有 optional dependency 机制自动取得匹配的 native platform package；模型、ORT/Windows ML、DirectML、CoreML bridge 或已接受的厂商 EP/runtime 必须随 release set 自包含交付。不得要求用户另装 CUDA、TensorRT、OpenVINO、VitisAI、QNN SDK 或编译工具链，也不得通过 install/postinstall 按硬件下载二进制。正常操作系统和硬件 driver 是唯一允许的系统前置条件。

Apple Silicon 已完成的 FP16 GPU、静态 ANE 和 W8A8 探针，以及建议的 ANE/GPU/CPU 分工，单独记录在 [Apple Device 加速技术方案](apple-device-acceleration.md)。Windows ML 2.x、DirectML 和 Intel/AMD/NVIDIA/Qualcomm provider 的分层策略记录在 [Windows Device 加速技术方案](windows-device-acceleration.md)。两份文档仍是 Draft；其中 spike 和上游能力不自动成为 accepted decision 或发布承诺。
...
| --- | --- | --- | --- | --- |
| Windows ML / DirectML | DirectML 通过 DirectX 12 覆盖 NVIDIA、AMD、Intel 和 Qualcomm 的多代 GPU；Windows ML 2.x 还能通过 EP ABI 装载厂商 GPU/NPU EP，并支持 pinned self-contained runtime | Windows x64；未来可扩 Windows arm64 | DirectML 已进入 legacy/maintenance；动态 catalog 下载/更新与本项目离线、固定版本原则冲突；npm 不能按 GPU vendor 过滤 dependency，稳定候选需验证 self-contained runtime + bundled bring-your-own EP 的体积、ABI 和许可 | Windows 混合 GPU 的技术横向覆盖最广；默认先验证 package 自带 CPU + DirectML，厂商 EP 只有通过用户加权收益与包体积 Gate 才进入同一 Windows release set |
...
- `sessionFallback` 默认 `error`，只控制 release manifest 中已声明的 descriptor 在 native load 前检查为当前设备不可用/不兼容，或 provider session 初始化失败时，是否改为 package 自带 CPU session；每个 session 的选择和原因都写入 `EngineInfo`。release payload 缺失、hash 不符或 DLL 无法加载属于 `package_load_failed`，不能伪装成正常硬件 fallback；
...
- `auto` 只能先作为 preview。它在首次 native load 前按 release manifest 的版本化顺序检查当前 platform package **明确携带**的 descriptor，只用 manifest/无副作用设备探测跳过缺失或明显不兼容项，然后为该进程/worker 选择一个 backend；一旦 native load 开始就冻结，不因 session 或运行失败再加载下一个 addon。所选 backend 内每个 session 是否落到 CPU 仍由 `sessionFallback`/`cpuPartition` 决定；运行期间不重新选择，也不扫描 `node_modules`、PATH、注册表或系统 SDK 目录猜测能力；
...
因此跨平台候选仍先评估 ONNX Runtime EP，而不是为了 GPU/NPU 立即换回完整 Paddle Inference。Apple provider 同时保留 Direct CoreML 候选；如果 ORT 无法达到同等 placement、shape、量化和资源表现，可以只替换内部 inference backend，不能复制 OCR pipeline。PaddleOCR/Paddle Inference 的官方结果作为候选和 benchmark 设计参考；只有 light-ocr 自己的模型派生物在相同 preprocess/postprocess、资源限制和结果 schema 下通过 PG，才形成产品能力。

### 7.7 Perf-2 — 按覆盖收益排序的宿主加速器 Spike

每个 provider 都是独立 Spike，可以接受、缩减或拒绝；一个 provider 失败不阻塞其他 provider 和主路线。

候选表不再使用固定优先级。Perf-0/HC 必须为每一轮生成排序、覆盖假设、样本量、置信度和目标设备；同分时先做可在现有 Tier 1 CI/开发设备上复现且离线分发成本更低的候选。内部可以产生不同 platform/provider artifact，但用户可见安装入口保持 `@arcships/light-ocr`；不能把选择或安装 runtime/EP 的责任转交给用户。
...
| Windows + bundled Windows ML bring-your-own EP / DirectML | Windows 跨厂商 GPU，以及可验证的厂商 NPU EP | 只评估随 npm release set 自带、不在首次运行下载的路径；比较 DirectML 与 WinML/vendor EP 的 API、包体积、device 选择、硬件代际和维护状态，并在无厂商 SDK 的干净 VM 验证 |
...
Windows 方向需要依据届时官方支持状态，在 Windows ML 的 bundled bring-your-own EP 与 DirectML 之间形成 D111 结论；任何会在首次运行下载或系统级自动更新 EP 的 catalog 路径均不接受。CUDA/TensorRT 如进入默认桌面能力，所需 redistributable 必须由 Windows release set 自带并通过安装大小 Gate；否则只能属于另一个明确立项的服务端产品，不能要求 `@arcships/light-ocr` 用户安装 CUDA/cuDNN/TensorRT。
...
└── npm-selected native platform package
    ├── baseline addon/runtime and CPU fallback
...
CLI 只暴露与 Node API 等价的 `--provider`、fallback 和 precision 语义；不提供要求用户定位 runtime 的 `--provider-package`、SDK path 或 DLL path。`auto` 只能在 release manifest 明确声明且当前 platform package 实际携带的 provider 中选择。

native 加载所有权必须遵守：

1. JS facade 保持 backend-neutral 和 lazy，在 provider 选择完成前不加载不兼容 runtime；
2. 优先使用同一份 ORT/Windows ML runtime 的 EP ABI plugin；每个进程/worker 首版只加载一种兼容 native backend；
3. provider 在 native load 前缺失/不兼容时，`sessionFallback: 'cpu'` 可以选择 package 自带 CPU backend；一旦 provider native load 已开始并失败，返回稳定错误，不在同一进程尝试注入第二份不兼容 runtime；
4. 厂商最优路径如果只能使用独立 runtime，必须封装成 package 内部 worker/backend，并通过 IPC、关闭、崩溃、资源和供应链 Gate；用户仍不安装厂商 runtime；
5. 若同一应用必须同时使用不同不兼容 provider，首版通过内部独立进程隔离，直到 D111 能证明多 runtime 共存安全。
...
- provider 请求与 release manifest/package 能力不匹配时返回稳定的 `package_load_failed` 或 `unsupported_capability`，只按上述 `sessionFallback` 状态机回落；
...
Perf-3 的目标是让用户只安装一个 OS 无关 package，同时可以显式选择或由受控 `auto` 选择当前 package 已自带的 accelerator，并从 `engine.info()`/CLI `info` 验证实际执行路径。
