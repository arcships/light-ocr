...
状态：工程实现完成（2026-07-22）。Small `0.4.0` 是稳定候选；Tiny/Medium `0.1.0` 只进入 `next`。G2 是后续 promotion 判断，不再作为 N2 代码完成或 N3 启动的机械阻塞项。
...
5. tiny/medium 发布到 `next`，只在实际受众证据证明价值后提升 stable；
