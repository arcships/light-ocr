...
- 四个平台分别保存三个 locked 2048² fixture 的 Core-vs-Python latency、Core diagnostics on/off lifetime peak，以及 Node 22/24 diagnostics on/off latency/peak；汇总 job 拒绝缺失矩阵、错误 identity、relative gate、absolute gate 与 accepted baseline 的 15% regression。
- 六个 tarball 先发布到一次性 Verdaccio registry，只安装 facade 后停止 registry，再执行真实 bounded 与 tiled OCR，证明没有运行时下载依赖。
- 只有以上 gates、人工受审 baseline 和 `publish_to_registry=true` 同时满足时，`npm-release` GitHub environment 才能读取 `NPM_TOKEN`；先发布五个依赖到 `next`，通过 registry facade 安装后再发布主包，最终禁网运行并可显式提升到 `latest`。
...
首次四平台 run 成功后，下载 `tiled-release-candidate-<commit>` artifact，人工检查 runner、波动、结果 hash 和 36 行完整矩阵，再显式批准：

```bash
python tools/tiled_release_gate.py approve \
  --candidate reports/tiled-platform-baselines.candidate.json \
  --approved-by-commit "$(git rev-parse HEAD)" \
  --output contracts/tiled-platform-baselines.json
```

accepted baseline 必须作为源码提交 review；脚本不会自动接受当前值。提交并重跑相同门禁通过后，才可把 `publish_to_registry` 设为 `true`。`promote_latest` 默认为 `false`，需要在 registry evidence 人工核对后显式选择。
