...
- 0.2.1 候选先在 Linux/Python 3.11 的哈希锁工具链中重现并校验 WebGPU FP16 ONNX，再在 macOS/Python 3.12 中派生固定 Core ML FP16 package hashes，最后把合并的 native superset bundle 交给 Linux assemble；用户安装、postinstall 和首次运行都不会执行转换或联网。
