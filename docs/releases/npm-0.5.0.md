...
`0.5.0` 完成 N3 文档入口功能。Small facade 新增 `recognizeDocument()` API
...
| --- | --- | --- | --- |
| Stable facade | `@arcships/light-ocr@0.5.0` | + `recognizeDocument` / `hasPdfSupport` / `document` CLI / `doctor` CLI | `next` → `latest` |
| Shared runtime | `@arcships/light-ocr-runtime@0.1.0` | + `loadNative` 导出 | 不变 |
