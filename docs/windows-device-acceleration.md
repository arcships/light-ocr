...
- **WebGPU 是 Windows x64 的通用 GPU 默认方向。** 当前同时验证锁定的 FP32 ONNX 与显式 native-FP16-I/O 派生模型，严格检查算子、shape、placement、CPU 释放、驱动和端到端收益；通过 Windows Gate 并随包交付后，D112 Auto 顺序为 `webgpu → cpu`，但首版 Auto 为兼容性仍使用 FP32。
...
1. **Native WebGPU FP32/FP16 实现完成：** ORT/plugin 与 Dawn/D3D12 companion 已精确锁定，FP16 派生模型可复现且源/输出哈希固定，C++/Node precision 路由、bounded CPU partition、strict fail-closed、profiling、descriptor、npm staging 与 offline verifier 已接线；真机必须验证 placement、动态 shape、质量、CPU-s、driver、内存和收益。
...
| --- | --- | --- | --- | --- |
| 具备合格 D3D12 adapter 的 x64 GPU | Native WebGPU | Auto/基线 FP32；显式 FP16 | 产品实现、qualification package 与 CI 已完成；尚待 Windows 真机 PG | ShaderF16、WebGPU kernel/shape、三类 CPU partition、Dawn、driver、copy、包体 |
...
| Native WebGPU FP16 派生模型 | 待测 | 单列 | 待测 | 待测 | 待测 | 待测 |
...
- 已实现 FP32 与显式 FP16 detector/recognizer 路由、bounded allow、strict fail-closed、Auto、C++/Node diagnostics、profiling 和 14-fixture one-command runner。
- 已实现从实际 payload 生成的 npm descriptor、license/SBOM、sterile cwd load 和离线 runtime/package cache 复装。
- 待用户在真实 Windows GPU 上运行 Gate；显式 FP32/FP16 allow 证明 placement，strict 证明稳定拒绝，Auto 必须实际选择 WebGPU。
...
1. 用户的 Windows GPU/driver 是否通过 FP32/FP16 placement、strict rejection、FP16 质量、冷启动、性能、RSS 和 lifecycle Gate？
2. 最低 Windows build、D3D12 driver、feature/limit 与设备族如何写入 compatibility range？
3. Recognition 的 `Concat/Gather/Slice` 有界 CPU partition 是否可接受，最低 placement coverage 与产品文案如何锁定？
