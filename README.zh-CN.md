...
> **`main` 上的 Apple 加速：**即将发布的 `0.2.1` 源码候选加入 macOS 15+ 可选 Direct Core ML 路径。在已完成资格验证的 Apple M4 Max 上，相比 12 线程 CPU profile，两个锁定 workload 取得 **2.30×–2.85× 的预热端到端加速**，OCR 进程 CPU time 降低 **95.91%–97.67%**。该 provider 尚未进入已经发布的 `0.2.0` npm packages。

> **PR #11 的 Native WebGPU 候选：**Linux x64/Vulkan 与 Windows x64/D3D12 已完成基于 ONNX Runtime 1.24.4 和官方 WebGPU Plugin EP 0.1.0 的自包含、可复现源码实现，包括显式 FP32/FP16 模型路由、有界 CPU partition、strict fail-closed、provider 注册、Auto fallback、runtime descriptor、npm staging、离线完整性校验与硬件无关测试。Linux/Windows 的 FP16 真机资格验证仍待完成，因此当前不宣称兼容范围或加速数据，也未进入已发布的 `0.2.0` packages。
...
- **按需启用原生 Apple 加速。**`0.2.1` 源码候选可以用 Core ML 执行 FP16 detection/recognition，同时保持 CPU 默认行为和公共 OCR 结果契约不变。
- **可复现的 Native WebGPU 候选。**PR #11 会打包官方 WebGPU Plugin EP 及其精确的 Linux/Vulkan 或 Windows/D3D12 运行时闭包，支持哈希校验的离线 staging 与一条命令完成的真机资格验证。
...
`0.2.1` provider Gate 在一台 Apple M4 Max（16 核 CPU、128 GB 内存，macOS 26.5.1）上，对比了显式启用的 FP16 Core ML 路径和 `cpu_fast` profile。CPU profile 最多使用 12 个 intra-op threads；每个 workload 先预热 5 次，再进行 3 组独立的 30 次测量。CPU time 降幅表示宿主进程占用变化，不是能耗数据。
...
```ts
const engine = await createEngine({
  // 仅源码候选需要；计划中的 npm 0.2.1 package 会自动解析该 payload。
...
Direct Core ML 加速已经合并到 `main`，目标版本为 `0.2.1`，但尚未进入已发布的 `0.2.0` package set。它会继续复用现有六包安装结构，不计划新增 provider package 或运行时下载。

PR #11 同时包含 Linux x64 与 Windows x64 Native WebGPU 源码候选。显式 FP16 会选择哈希锁定、原生 FP16 图输入/输出的派生模型；Auto 为兼容性仍使用 FP32，三个必要 CPU partition 算子会被显式报告并限制范围。其 npm payload 仍受两个平台真机资格报告的发布门槛约束；报告完成审阅前，已发布的 `0.2.0` packages 保持不变，并在这两个平台继续仅使用 CPU。

## 项目状态

`light-ocr` 仍在积极开发。`0.2.0` 已发布确定性的 `tiled-v1` 大图模式，以及 Node.js 适配器中受资源限制的内存 JPEG/PNG 解码；C++ Core 的 raw-pixel 边界保持不变。当前源码候选加入 macOS 可选 Direct Core ML，以及 Linux x64/Windows x64 Native WebGPU 执行；各平台 Auto 默认值与发布支持范围仍由对应资格证据决定。
